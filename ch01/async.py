...
def get_all_users_paginated(start: int, count: int):
    ...
    for index in range(0, len(keys)):
        ...
        user = await get_user_by_id(current_user_key)
...
async def user_info_endpoint():
    full_user_profile = await get_user_by_id()
...
async def get_user_by_id_endpoint(user_id: int):
    full_user_profile = await get_user_by_id(user_id)
...
async def add_user_endpoint(full_profile_info: FullUserProfile):
    user_id = await create_or_update_user(full_profile_info)
...
async def update_or_create_user_endpoint(user_id: int, full_user_profile: FullUserProfile):
    await create_or_update_user(profile=full_user_profile, new_user_id=user_id)
