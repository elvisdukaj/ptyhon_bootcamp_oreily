...
def create_or_update_user(profile: FullUserProfile, user_id: Optional[int] = None) -> int:
    """
    Create a user is new_user_id exists otherwise update an existing user
    Placeholder implementation to be updated with database

    :param profile: FullUserProfile - FullUserProfile to add or update
    :param user_id: Optional[int] - if exists is updating an existing user otherwise it creates a new one
    ...
    users_content[user_id] = {"username": username, "liked_posts": liked_post}
    profiles_info[user_id] = {
        "short_bio": short_bio,
        "long_bio": long_bio
    }
...
def get_user_by_id_endpoint(user_id: int):
    """
    Endpoint for getting the end user by user_id


    :param user_id: int - unique and monotonically increasing
...
def update_or_create_user_endpoint(user_id: int, full_user_profile: FullUserProfile):
    create_or_update_user(profile=full_user_profile, user_id=user_id)
